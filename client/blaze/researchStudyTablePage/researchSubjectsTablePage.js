...
Router.map(function(){
  this.route('researchStudyTablePage', {
    path: '/table/researchStudy',
    template: 'researchStudyTablePage'
  });
});
...
Template.researchStudyTablePage.events({
  'click .checkbox': function(event, template){
    event.stopPropagation();
    if(this.checked){
      ResearchStudy.update({_id: this._id}, {$set: {checked: false}});
    }else{
      ResearchStudy.update({_id: this._id}, {$set: {checked: true}});
    }
  },
  'click .flag': function(event, template){
    event.stopPropagation();
    if(this.flagged){
      ResearchStudy.update({_id: this._id}, {$set: {flagged: false}});
    }else{
      ResearchStudy.update({_id: this._id}, {$set: {flagged: true}});
    }
  },
...
Template.researchStudyTablePage.helpers({
  getName: function(){
...
Template.researchStudyTablePage.rendered = function(){

  // step A:  initialize the table sorting functionality
  $(this.find('#researchStudyTable')).tablesorter();
  ...
  Tracker.autorun(function() {
    ...
    setTimeout(function() {
      ...
      $("#researchStudyTable").trigger("update");
    }, 200);
  });
