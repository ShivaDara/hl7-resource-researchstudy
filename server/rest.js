  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (req.body) {
        ...
        researchStudyUpdate = ResearchStudy.toMongo(researchStudyUpdate);
        researchStudyUpdate = ResearchStudy.prepForUpdate(researchStudyUpdate);


        process.env.DEBUG && console.log('-----------------------------------------------------------');
        process.env.DEBUG && console.log('researchStudyUpdate', JSON.stringify(researchStudyUpdate, null, 2));

        var researchStudy = ResearchStudy.findOne(req.params.id);
        var researchStudyId;

        if(researchStudy){
          process.env.DEBUG && console.log('ResearchStudy found...')
          researchStudyId = ResearchStudy.update({_id: req.params.id}, {$set: researchStudyUpdate },  function(error, result){
            if (error) {
            ...
            if (result) {
              ...
              var researchStudy = ResearchStudy.find({_id: req.params.id});
              var payload = [];

              researchStudy.forEach(function(record){
                payload.push(ResearchStudy.prepForFhirTransfer(record));
              });
          ...
          researchStudyId = ResearchStudy.insert(researchStudyUpdate,  function(error, result){
            if (error) {
              process.env.TRACE && console.log('PUT /fhir/ResearchStudy/' + req.params.id + "[error]", error);
...
JsonRoutes.add("get", "/" + fhirVersion + "/ResearchStudy/:id", function (req, res, next) {
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      var researchStudyData = ResearchStudy.findOne({_id: req.params.id});
      if (researchStudyData) {
        ...
        JsonRoutes.sendResult(res, {
          code: 200,
          data: ResearchStudy.prepForFhirTransfer(researchStudyData)
        });
      } else {
        // Gone
        JsonRoutes.sendResult(res, {
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (req.body) {
        ...
        newResearchStudy = ResearchStudy.toMongo(newResearchStudy);
        ...
        ResearchStudySchema.clean(newResearchStudy);

        var practionerContext = ResearchStudySchema.newContext();
        practionerContext.validate(newResearchStudy)
        console.log('New researchStudy is valid:', practionerContext.isValid());
        console.log('check', check(newResearchStudy, ResearchStudySchema))
        


        var researchStudyId = ResearchStudy.insert(newResearchStudy,  function(error, result){
          if (error) {
          ...
          if (result) {
            process.env.TRACE && console.log('result', result);
            res.setHeader("ResearchStudy", "fhir-3.0.0/ResearchStudy/" + result);
            res.setHeader("Last-Modified", new Date());
            res.setHeader("ETag", "3.0.0");

            var researchStudy = ResearchStudy.find({_id: result});
            var payload = [];
...
JsonRoutes.add("get", "/" + fhirVersion + "/ResearchStudy", function (req, res, next) {
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      var payload = [];
      var researchStudy = ResearchStudy.find(databaseQuery);
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (req.params.param.includes('_search')) {
        ...
        researchStudy = ResearchStudy.find(databaseQuery, {limit: searchLimit});
      ...
      //process.env.TRACE && console.log('researchStudy', researchStudy);
...
JsonRoutes.add("delete", "/" + fhirVersion + "/ResearchStudy/:id", function (req, res, next) {
  ...
  if(typeof oAuth2Server === 'object'){
    ...
    if (accessToken || process.env.NOAUTH || Meteor.settings.private.disableOauth) {
      ...
      if (ResearchStudy.find({_id: req.params.id}).count() === 0) {
        // No Content
        JsonRoutes.sendResult(res, {
          code: 204
        });
      } else {
        ResearchStudy.remove({_id: req.params.id}, function(error, result){
          if (result) {
            // No Content
            JsonRoutes.sendResult(res, {
              code: 204
